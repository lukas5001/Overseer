...
export default function VariableBar({ variables, values, onChange, fixedVariables }: VariableBarProps) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 border-b border-gray-800 bg-gray-900/60 flex-shrink-0 flex-wrap">
      {variables.map(v => (
        <VariableDropdown
          key={v.name}
          variable={v}
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {!disabled && <ChevronDown className="w-3 h-3 text-gray-500" />}
      </button>

      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
          <div className="absolute top-full left-0 mt-1 z-50 bg-gray-800 border border-gray-600 rounded-lg shadow-xl min-w-[180px] max-h-60 overflow-y-auto">
            {allOptions.map(opt => {
            ...
            {allOptions.length === 0 && (
              <p className="text-xs text-gray-500 p-3 text-center">Keine Optionen</p>
            )}
          </div>
        </>
      )}
