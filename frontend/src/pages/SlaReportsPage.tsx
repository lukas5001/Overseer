    ...
    <div className="p-8">
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          {periodOptions.map(p => (
            <button key={p.days} onClick={() => { setDays(p.days); setCustomStart(''); setCustomEnd('') }}
              className={clsx('px-3 py-2 rounded-lg text-sm font-medium transition-colors',
                days === p.days && !customStart ? 'bg-overseer-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}>
              {p.label}
            </button>
          ))}
        </div>
        ...
        <button onClick={() => exportCsv(services, tenantName, `${start}_${end}`)}
          ...
          className="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 text-gray-700 text-sm rounded-lg hover:bg-gray-50 disabled:opacity-40 ml-auto">
      ...
      {/* Table */}
      {isLoading ? (
        <div className="text-gray-400 text-sm">Lade SLA-Daten…</div>
      ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {services.map(s => (
                <tr key={s.service_id} className="hover:bg-gray-50">
                  <td className="px-6 py-3 font-medium text-gray-900">{s.service_name}</td>
                  <td className="px-6 py-3 text-gray-500">{s.host_name}</td>
                  <td className={clsx('px-6 py-3 text-right font-bold', slaColor(s.sla_pct))}>
                    {s.sla_pct != null ? `${s.sla_pct.toFixed(2)}%` : 'N/A'}
                  </td>
                  <td className="px-6 py-3 text-right text-gray-600">{s.uptime_minutes.toLocaleString()}</td>
                  <td className="px-6 py-3 text-right text-gray-600">{s.downtime_minutes.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {services.length === 0 && (
            <div className="p-8 text-center text-gray-400 text-sm">Keine SLA-Daten für den gewählten Zeitraum.</div>
          )}
        </div>
      )}
    </div>
