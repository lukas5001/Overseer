...
export default function CustomDashboardViewPage() {
  ...
  if (!dashboard) return <div className="p-6 text-gray-500 dark:text-gray-400">Dashboard nicht gefunden</div>
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 flex-shrink-0">
        <div className="flex items-center gap-3">
          <Link to="/custom-dashboards" className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
          ...
          {isEditing ? (
            <input
              ...
              className="text-lg font-bold text-gray-900 dark:text-white bg-transparent border-b border-gray-300 dark:border-gray-600 focus:border-blue-500 outline-none px-1 py-0.5"
            />
          ) : (
            <h1 className="text-lg font-bold text-gray-900 dark:text-white">{dashboard.title}</h1>
          )}
          ...
          {!isEditing && (
            <div className="flex items-center gap-1.5 ml-2">
              ...
              <select
                ...
                className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md px-2 py-1 text-xs text-gray-600 dark:text-gray-300"
              >
                ...
                {isEditing && (
                  <>
                    <button
                      ...
                      className="absolute top-1 right-8 w-6 h-6 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20 hover:bg-gray-200 dark:hover:bg-gray-600"
      ...
      {showVersions && (
        <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setShowVersions(false)}>
          ...
          <div className="relative w-80 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 h-full overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Versionen</h3>
              <button onClick={() => setShowVersions(false)} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-3 space-y-2">
              {versions?.map(v => (
                <div key={v.id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-900 rounded-lg px-3 py-2">
                  <div>
                    <span className="text-sm text-gray-900 dark:text-white">Version {v.version}</span>
              ...
              {(!versions || versions.length === 0) && (
                <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-4">Keine Versionen</p>
              )}
            </div>
          </div>
        </div>
      ...
      {showSettings && isEditing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowSettings(false)}>
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl w-full max-w-lg max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-base font-semibold text-gray-900 dark:text-white">Dashboard-Einstellungen</h2>
              <button onClick={() => setShowSettings(false)} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex px-4 pt-3 gap-1">
              <button
                ...
                className={clsx(
                  ...
                  settingsTab === 'general' ? 'bg-blue-600 text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white bg-gray-100 dark:bg-gray-700'
                )}
              >
                Allgemein
              </button>
              <button
                ...
                className={clsx(
                  'px-3 py-1.5 text-xs rounded-md transition-colors',
                  settingsTab === 'variables' ? 'bg-blue-600 text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white bg-gray-100 dark:bg-gray-700'
                )}
              >
                Variablen
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {settingsTab === 'general' ? (
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="block text-sm text-gray-500 dark:text-gray-400 mb-1">Beschreibung</label>
                    <input
                      value={editDesc}
