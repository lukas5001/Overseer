...
const STATUS_OPTIONS: { value: CheckStatus; label: string; color: string }[] = [
  ...
  { value: 'WARNING', label: 'Warning', color: 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300' },
  { value: 'UNKNOWN', label: 'Unknown', color: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300' },
]
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-xl mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{existing ? 'Regel bearbeiten' : 'Neue Alert-Regel'}</h2>
        ...
        <div className="space-y-4">
          ...
          {/* Status Checkboxes */}
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">Status-Filter</label>
              ...
              {STATUS_OPTIONS.map(opt => (
                <button key={opt.value} onClick={() => toggleStatus(opt.value)}
                  className={clsx('px-3 py-1.5 rounded text-xs font-semibold transition-all',
                    statuses.includes(opt.value) ? opt.color : 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500')}>
                  {opt.label}
                </button>
              ))}
            </div>
          ...
          {/* Host Tags */}
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Host-Tags (kommagetrennt)</label>
          ...
          {/* Service Names */}
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Service-Namen (kommagetrennt)</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">Benachrichtigungskanäle</label>
              ...
              {channels.filter(c => c.active).map(ch => (
                <label key={ch.id} className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={selectedChannels.includes(ch.id)}
                    onChange={() => toggleChannel(ch.id)} className="rounded border-gray-300 dark:border-gray-600" />
                  <span className="text-gray-700 dark:text-gray-300">{ch.name}</span>
                  <span className="text-xs text-gray-400 dark:text-gray-500">({ch.channel_type})</span>
                </label>
              ))}
              {channels.length === 0 && <p className="text-xs text-gray-400 dark:text-gray-500">Keine Kanäle konfiguriert</p>}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400">Eskalationsstufen</label>
            ...
            {steps.length === 0 && (
              <p className="text-xs text-gray-400 dark:text-gray-500">Keine Eskalation konfiguriert</p>
            )}
            <div className="space-y-2">
              {steps.map((step, i) => (
                <div key={i} className="flex items-start gap-2 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                  <div className="flex flex-col gap-1">
                    <button onClick={() => moveStep(i, -1)} disabled={i === 0} className="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 disabled:opacity-30">
                      <ChevronUp className="w-3 h-3" />
                    </button>
                    <button onClick={() => moveStep(i, 1)} disabled={i === steps.length - 1} className="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 disabled:opacity-30">
                      <ChevronDown className="w-3 h-3" />
                    </button>
                  </div>
                  <div className="flex-1 space-y-2">
                    <div>
                      <label className="text-xs text-gray-500 dark:text-gray-400">Verzögerung (Min.)</label>
                      <input type="number" min={0} value={step.delay_minutes}
                        onChange={e => updateStep(i, 'delay_minutes', parseInt(e.target.value) || 0)}
                        className="w-full text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded px-2 py-1 outline-none focus:ring-1 focus:ring-overseer-500" />
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 dark:text-gray-400">Kanäle</label>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {channels.filter(c => c.active).map(ch => (
                          <button key={ch.id} onClick={() => {
                            ...
                            step.channels.includes(ch.id) ? 'bg-overseer-100 text-overseer-700' : 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400')}>
                            {ch.name}
                          </button>
                        ))}
                      </div>
                  ...
                  <button onClick={() => removeStep(i)} className="text-gray-400 dark:text-gray-500 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          </div>
...
function GroupingSettingsPanel({ tenants }: { tenants: Tenant[] }) {
  ...
  if (!settings) return isLoading ? <div className="text-gray-400 dark:text-gray-500 text-sm">Lade…</div> : null

  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
      <div className="flex items-center gap-2 mb-4">
        <Layers className="w-5 h-5 text-overseer-600" />
        <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Alert Grouping</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl">
        ...
        {tenants.length > 1 && (
          <div className="md:col-span-2">
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Tenant</label>
            <select value={tenantId} onChange={e => setTenantId(e.target.value)}
              className="w-full text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-overseer-500 max-w-xs">
              {tenants.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
            </select>
          </div>
        )}
        ...
        <div className="md:col-span-2 flex items-center gap-3">
          <button
            ...
            className={clsx('w-10 h-5 rounded-full relative transition-colors',
              settings.enabled ? 'bg-overseer-600' : 'bg-gray-300 dark:bg-gray-600')}>
          ...
          <span className="text-sm text-gray-700 dark:text-gray-300">Grouping {settings.enabled ? 'aktiv' : 'deaktiviert'}</span>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-xl mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{existing ? 'Log-Regel bearbeiten' : 'Neue Log-Alert-Regel'}</h2>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={pattern} onChange={e => setPattern(e.target.value)}
              ...
              className="w-full text-sm font-mono border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-overseer-500" />
          ...
          {/* Condition Type */}
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">Bedingung</label>
          ...
          {conditionType === 'threshold' && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Schwellwert (Treffer)</label>
                <input type="number" min={1} value={thresholdCount} onChange={e => setThresholdCount(parseInt(e.target.value) || 1)}
                  className="w-full text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-overseer-500" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Zeitfenster</label>
          ...
          {conditionType === 'absence' && (
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Erwartetes Intervall</label>
              <select value={timeWindow} onChange={e => setTimeWindow(parseInt(e.target.value))}
                className="w-full text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-overseer-500">
                <option value={300}>5 Minuten</option>
                <option value={600}>10 Minuten</option>
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Alert wenn das Pattern nicht innerhalb dieses Zeitraums erscheint.</p>
            </div>
          )}

          {/* Alert Severity */}
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">Alert-Severity</label>
            <div className="flex gap-2">
              <button onClick={() => setAlertSeverity('CRITICAL')}
                className={clsx('px-3 py-1.5 rounded text-xs font-semibold',
                  alertSeverity === 'CRITICAL' ? 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300' : 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500')}>
                Critical
              </button>
              <button onClick={() => setAlertSeverity('WARNING')}
                className={clsx('px-3 py-1.5 rounded text-xs font-semibold',
                  alertSeverity === 'WARNING' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300' : 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500')}>
                Warning
              </button>
            </div>
          </div>
          ...
          {/* Scope: Hosts */}
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Host-IDs (kommagetrennt, leer = alle)</label>
          ...
          {/* Scope: Services */}
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Services (kommagetrennt, leer = alle)</label>
          ...
          {/* Min Severity Filter */}
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Mindest-Log-Severity</label>
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Alert-Regeln</h1>
          <span className="text-sm text-gray-500 dark:text-gray-400 ml-2">{rules.length} Regeln</span>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {rules.map(rule => (
              <tr key={rule.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-6 py-3 font-medium text-gray-900 dark:text-gray-100">{rule.name}</td>
                <td className="px-6 py-3">
                  <div className="flex gap-1 flex-wrap">
                    {rule.conditions.statuses.map(s => (
                      <span key={s} className={clsx('px-1.5 py-0.5 rounded text-xs font-semibold',
                        s === 'CRITICAL' ? 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300' : s === 'WARNING' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300')}>
                        {s}
                      </span>
                    ))}
                    <span className="text-xs text-gray-400 dark:text-gray-500 ml-1">{rule.conditions.min_duration_minutes}min</span>
                  </div>
                </td>
                <td className="px-6 py-3 text-center text-gray-500 dark:text-gray-400">{rule.notification_channels.length}</td>
                <td className="px-6 py-3 text-xs text-gray-500 dark:text-gray-400">{tenantNames[rule.tenant_id] ?? '–'}</td>
                <td className="px-6 py-3 text-center">
                  <button
                    onClick={() => toggleEnabled.mutate({ id: rule.id, enabled: !rule.enabled })}
                    className={clsx('w-10 h-5 rounded-full relative transition-colors',
                      rule.enabled ? 'bg-overseer-600' : 'bg-gray-300 dark:bg-gray-600')}>
                    <span className={clsx('absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform',
                ...
                <td className="px-6 py-3 text-right">
                  <div className="flex items-center gap-2 justify-end">
                    <button onClick={() => testRule.mutate(rule.id)} title="Test" className="text-gray-400 dark:text-gray-500 hover:text-overseer-600">
                      <TestTube2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => setEditRule(rule)} title="Bearbeiten" className="text-gray-400 dark:text-gray-500 hover:text-overseer-600">
        ...
        {rules.length === 0 && !isLoading && (
          <div className="p-8 text-center text-gray-400 dark:text-gray-500 text-sm">Keine Alert-Regeln konfiguriert.</div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between mt-10 mb-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Log-Alert-Regeln</h2>
          <span className="text-sm text-gray-500 dark:text-gray-400">{logRules.length} Regeln</span>
        </div>
              ...
              <tr key={rule.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td className="px-6 py-3 text-center">
                  <span className={clsx('px-1.5 py-0.5 rounded text-xs font-semibold',
                    rule.alert_severity === 'CRITICAL' ? 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300')}>
                    {rule.alert_severity}
                  </span>
                </td>
                <td className="px-6 py-3 text-center text-gray-500 dark:text-gray-400">{rule.notification_channels.length}</td>
                ...
                <td className="px-6 py-3 text-right">
                  <div className="flex items-center gap-2 justify-end">
                    <button onClick={() => setEditLogRule(rule)} title="Bearbeiten" className="text-gray-400 dark:text-gray-500 hover:text-overseer-600">
                      <Zap className="w-4 h-4" />
                    </button>
                    <button onClick={() => setDeleteLogTarget(rule.id)} title="Löschen" className="text-gray-400 dark:text-gray-500 hover:text-red-500">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </td>
