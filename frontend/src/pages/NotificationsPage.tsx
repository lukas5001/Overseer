        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://hooks.example.com/..."
              className="w-full text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-overseer-500 outline-none font-mono text-xs" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Secret (optional)</label>
    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Benachrichtigungen</h1>
          <span className="text-sm text-gray-500 dark:text-gray-400">{activeChannels.length} aktiv</span>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 dark:text-gray-400">Noch keine Webhooks konfiguriert.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {activeChannels.map(ch => (
            <div key={ch.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-5 py-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-medium text-gray-800 dark:text-gray-200 text-sm">{ch.name}</p>
                ...
                <div className="flex items-center gap-2 mt-1">
                  {ch.events.map(ev => (
                    <span key={ev} className="text-xs bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-1.5 py-0.5 rounded">
                      {ev}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-xs px-3 py-1.5 rounded border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 font-medium"
                >
                  Bearbeiten
                </button>
                <button
      ...
      {/* Payload example */}
      <div className="mt-8 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
        <h3 className="font-semibold text-gray-800 dark:text-gray-200 text-sm mb-3">Webhook-Payload Beispiel</h3>
        <pre className="text-xs text-gray-600 dark:text-gray-400 font-mono bg-white dark:bg-gray-800 rounded-lg p-4 overflow-x-auto border border-gray-100 dark:border-gray-800">{`{
  "event": "state_change",
