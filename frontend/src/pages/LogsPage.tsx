...
const SEVERITY_CONFIG: Record<number, { label: string; bg: string; text: string; short: string }> = {
  0: { label: 'Emergency',  bg: 'bg-purple-100 dark:bg-purple-900/40',  text: 'text-purple-800 dark:text-purple-300',  short: 'EMERG' },
  1: { label: 'Alert',      bg: 'bg-red-100 dark:bg-red-900/40',        text: 'text-red-800 dark:text-red-300',        short: 'ALERT' },
  2: { label: 'Critical',   bg: 'bg-red-100 dark:bg-red-900/40',        text: 'text-red-800 dark:text-red-300',        short: 'CRIT' },
  3: { label: 'Error',      bg: 'bg-red-100 dark:bg-red-900/40',        text: 'text-red-800 dark:text-red-300',        short: 'ERROR' },
  4: { label: 'Warning',    bg: 'bg-amber-100 dark:bg-amber-900/40',    text: 'text-amber-800 dark:text-amber-300',    short: 'WARN' },
  5: { label: 'Notice',     bg: 'bg-blue-100 dark:bg-blue-900/40',      text: 'text-blue-800 dark:text-blue-300',      short: 'NOTICE' },
  6: { label: 'Info',       bg: 'bg-sky-100 dark:bg-sky-900/40',        text: 'text-sky-800 dark:text-sky-300',        short: 'INFO' },
  7: { label: 'Debug',      bg: 'bg-gray-100 dark:bg-gray-700',         text: 'text-gray-800 dark:text-gray-300',      short: 'DEBUG' },
}
    ...
    <div className="p-4 sm:p-6 max-w-[1600px] mx-auto">
      ...
      <form onSubmit={handleSearch} className="mb-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-24 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
          <button
            type="submit"
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <div className="relative" ref={hostFilterRef}>
          ...
          {showHostFilter && (
            <div className="absolute z-20 top-full mt-1 left-0 w-64 max-h-64 overflow-y-auto bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl">
              {selectedHosts.length > 0 && (
                <button
                  onClick={() => setSelectedHosts([])}
                  className="w-full px-3 py-1.5 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white text-left border-b border-gray-200 dark:border-gray-700"
                >
                  Alle abwählen
                </button>
              )}
              {(hosts ?? []).map(h => (
                <label key={h.id} className="flex items-center gap-2 px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer text-sm text-gray-700 dark:text-gray-200">
                  <input
                    type="checkbox"
                    checked={selectedHosts.includes(h.id)}
                    onChange={() => toggleHost(h.id)}
                    className="rounded border-gray-300 dark:border-gray-600 text-blue-500 focus:ring-blue-500 bg-white dark:bg-gray-700"
                  />
        ...
        <div className="relative" ref={severityFilterRef}>
          ...
          {showSeverityFilter && (
            <div className="absolute z-20 top-full mt-1 left-0 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl">
              {selectedSeverities.length > 0 && (
                <button
              ...
              {Object.entries(SEVERITY_CONFIG).map(([sev, cfg]) => (
                <label key={sev} className="flex items-center gap-2 px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer text-sm">
                  <input
                    type="checkbox"
        ...
        {/* Time Range */}
        <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-0.5">
          {TIME_RANGES.map(tr => (
            <button
              key={tr.value}
        ...
        {isLoading && allLogs.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-800/50">
            {logsWithDates.map((item) => {
              if (item.type === 'date') {
                return (
                  <div key={`date-${item.date}`} className="px-4 py-1.5 bg-gray-100 dark:bg-gray-800/50 text-xs text-gray-500 dark:text-gray-500 font-medium sticky top-0 z-10">
                    {item.date}
                  </div>
                )
              }
              ...
              return (
                <div key={`${log.time}-${idx}`}>
                  <button
                    ...
                    className={clsx(
                      'w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors group',
                      isExpanded && 'bg-gray-100/50 dark:bg-gray-800/30'
                    )}
                  >
                    <div className="flex items-start gap-3">
                      {/* Expand icon */}
                      {isExpanded
                        ? <ChevronDown className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500 mt-0.5 flex-shrink-0" />
                        : <ChevronRight className="w-3.5 h-3.5 text-gray-300 dark:text-gray-600 mt-0.5 flex-shrink-0 group-hover:text-gray-500 dark:group-hover:text-gray-400" />
                      }

                      {/* Timestamp */}
                      <span className="font-mono text-xs text-gray-500 dark:text-gray-500 flex-shrink-0 mt-0.5 w-[90px]">
                        {formatTimestamp(log.time)}
                      </span>

                      {/* Host + Service */}
                      <span className="text-xs text-gray-500 dark:text-gray-400 flex-shrink-0 mt-0.5 w-[180px] truncate">
                        {log.host ?? 'unknown'}
                        {log.service && <span className="text-gray-400 dark:text-gray-600"> / {log.service}</span>}
                      </span>
                      ...
                      <span
                        className={clsx(
                          ...
                          log.severity <= 3 ? 'text-red-600 dark:text-red-400' : log.severity === 4 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-700 dark:text-gray-300'
                        )}
                      >
                        <span
                    ...
                    <div className="px-4 pb-3 pl-[52px] space-y-2">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                        <div>
                          ...
                          <span className="text-gray-600 dark:text-gray-300 font-mono">{format(new Date(log.time), 'dd.MM.yyyy HH:mm:ss.SSS')}</span>
                        </div>
                        <div>
                        ...
                        {log.source_path && (
                          <div className="col-span-2">
                            ...
                            <span className="text-gray-600 dark:text-gray-300 font-mono">{log.source_path}</span>
                          </div>
                        )}
                        <div>
                          <span className="text-gray-500">Host:</span>{' '}
                          <span className="text-gray-600 dark:text-gray-300">{log.host}</span>
                        </div>
                        {log.service && (
                          <div>
                            <span className="text-gray-500">Service:</span>{' '}
                            <span className="text-gray-600 dark:text-gray-300">{log.service}</span>
                          </div>
                        )}
                        <div>
                      ...
                      <div className="mt-2">
                        ...
                        <pre className="mt-1 p-2 bg-gray-100 dark:bg-gray-950 rounded text-xs text-gray-700 dark:text-gray-300 font-mono whitespace-pre-wrap break-words max-h-60 overflow-auto">
                      ...
                      {log.fields && Object.keys(log.fields).length > 0 && (
                        <div className="mt-2">
                          ...
                          <div className="mt-1 p-2 bg-gray-100 dark:bg-gray-950 rounded text-xs font-mono">
      ...
      {allLogs.length < total && (
        <div className="flex justify-center mt-4">
          <button
            ...
            disabled={isLoading}
            className="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg text-sm transition-colors disabled:opacity-50"
          >
